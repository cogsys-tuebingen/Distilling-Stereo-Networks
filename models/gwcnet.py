...
class feature_extraction(nn.Module):
    ...
    def forward(self, x, being_taught=False):
        x = self.firstconv(x)
        x = self.layer1(x)
        l2 = self.layer2(x)
        l3 = self.layer3(l2)
...
class GwcNet(nn.Module):
    def __init__(self, maxdisp, loss_type, use_concat_volume=False):
    ...
    def forward(self, left, right, teaching=False):
        ...
        if self.training:
            ...
            prepred3 = F.softmax(cost3, dim=1)
            pred3 = disparity_regression(prepred3, self.maxdisp)
            ...
            if teaching:
                if self.loss_type == "2_feats":
                    return [pred3], features_left["gwc_feature"], features_right["gwc_feature"]
...
def GwcNet_G(d, lt=0, transform_feats=None):
    return GwcNet(d, loss_type=lt, use_concat_volume=False)
