...
def adjust_learning_rate(optimizer, epoch, base_lr, lrepochs, sweep=False):
    if sweep:
        downscale_epochs = lrepochs[0]
        downscale_rate = lrepochs[1]
    else:
...
def split_weight_epochs(weight_epochs):
    splits = weight_epochs.split(':')
    assert len(splits) == 2

    downscale_epochs = [int(eid_str) for eid_str in splits[0].split(',')]
    downscale_rate = float(splits[1])
    return downscale_epochs, downscale_rate
